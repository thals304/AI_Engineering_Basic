# -*- coding: utf-8 -*-
"""24차시_프로젝트 8 제출코드.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1nd_luTlEGEpViUAU77IrlUOzAHCNhW7v
...
def forward_loss(X, Y, _params):
  ...
  N = X.shape[1]  # 샘플 수
  # 첫번째 레이어연산
  Z1 = np.dot(W1, X) + B1 # 1) affine 함수  - '...'에 채워주세요
  H = 1 / (1 + np.exp(-Z1)) # 2) sigmoid 함수 - '...'에 채워주세요

  # 두번째 레이어 연산
  Z2 = np.dot(W2.T, H) + B2 # 3) affine 함수 - '...'에 채워주세요
  YHat = 1 / (1 + np.exp(-Z2)) # 4) sigmoid 함수  - '...'에 채워주세요

  # 손실함수 계산
  loss = -np.sum(Y * np.log(YHat) + (1 - Y) * np.log(1 - YHat)) / N  # 5) 이진크로스 엔트로피 함수 - '...'에 채워주세요
...
def optimize (X, Y, _params, learning_rate = 0.1, iteration = 1000):
    ...
    params = init_parameters(2)  # 파라미터 초기화 # params = np.copy(_params) # 파라미터 복사
    loss_trace = [] # 손실 값 저장
